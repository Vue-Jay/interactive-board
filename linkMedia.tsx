...
export function resolveLinkMedia(raw:string):LinkMediaInfo|null{
 ...
 if(id&&/^[A-Za-z0-9_-]{6,20}$/.test(id))return{kind:"youtube",sourceUrl:u.href,embedUrl:`https://www.youtube-nocookie.com/embed/${id}?rel=0&modestbranding=1`,host};
...
export function LinkMediaPlayer({item}: {item:Item}){
 ...
 if(info.kind==="youtube"||info.kind==="vimeo"){
  ...
  return <iframe className="link-media-frame" src={info.embedUrl} title={item.mediaTitle||"Видео"} allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen onPointerDown={e=>e.stopPropagation()}/>;
 }
