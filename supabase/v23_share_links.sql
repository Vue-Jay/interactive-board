-- InteractiveBoard v22/v23: protected share links.
...
begin
  ...
  values(p_board_id,encode(digest(v_token,'sha256'),'hex'),p_role,auth.uid())
  returning * into v_link;
  return jsonb_build_object('id',v_link.id,'board_id',v_link.board_id,'role',v_link.role,'created_at',v_link.created_at,'expires_at',v_link.expires_at,'token',v_token);
end $$;
...
begin
  if auth.uid() is null then raise exception 'Authentication required' using errcode='42501'; end if;
  if not public.is_board_owner(p_board_id) then raise exception 'Недостаточно прав'; end if;
  return coalesce((select jsonb_agg(jsonb_build_object('id',id,'board_id',board_id,'role',role,'created_at',created_at,'expires_at',expires_at,'revoked_at',revoked_at) order by created_at desc)
    from public.board_share_links where board_id=p_board_id and revoked_at is null),'[]'::jsonb);
end $$;
...
begin
  if auth.uid() is null then raise exception 'Authentication required' using errcode='42501'; end if;
  select board_id into v_board_id from public.board_share_links where id=p_link_id;
  if v_board_id is null or not public.is_board_owner(v_board_id) then raise exception 'Недостаточно прав'; end if;
  update public.board_share_links set revoked_at=now() where id=p_link_id and revoked_at is null;
end $$;

create or replace function public.redeem_board_share_link(p_token text)
returns jsonb language plpgsql security definer set search_path=public,extensions as $$
declare
  v_user uuid:=auth.uid(); v_link public.board_share_links%rowtype; v_board public.boards%rowtype;
  v_existing_role text; v_effective_role text;
begin
  ...
  if not found then raise exception 'Ссылка недействительна или отозвана'; end if;
