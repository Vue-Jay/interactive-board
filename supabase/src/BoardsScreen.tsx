...
const retention=(iso?:string|null)=>{if(!iso)return"";const days=Math.max(0,Math.ceil((Date.parse(iso)-Date.now())/86400000));return days===0?"очистка сегодня":`осталось ${days} дн.`};
...
export default function BoardsScreen({user,accountRole,isAppAdmin,onOpenBoard,onLogout:_onLogout}:Props){
 ...
 return <main className="boards-shell"><header className="boards-header"><div className="boards-brand"><div className="auth-logo" aria-label="OnlineRepetitor">OR</div><div><strong>OnlineRepetitor</strong><span>{accountRole==="teacher"?"Преподаватель":"Ученик"} · {isRemoteBackendEnabled()?"синхронизация включена":"локальный режим"}</span></div></div><div className="boards-account"><div className="account-chip"><span className="account-avatar">{user.name.charAt(0).toUpperCase()}</span><span className="account-copy"><strong>{user.name}</strong><small>{user.email}</small></span></div><button className="boards-secondary" onClick={()=>{window.history.pushState({},"","/?section=profile");window.dispatchEvent(new PopStateEvent("popstate"))}}>⚙ Настройки</button></div></header>

 <section className="boards-content"><details className="mobile-dashboard-menu"><summary><span>☰</span><b>Разделы</b><small>Доски</small></summary><div className="mobile-dashboard-menu-grid"><button className="active">Доски</button>{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({}, "", "/?section=students");window.dispatchEvent(new PopStateEvent("popstate"))}}>Ученики</button>}<button onClick={()=>{window.history.pushState({}, "", "/?section=assignments");window.dispatchEvent(new PopStateEvent("popstate"))}}>Задания</button>{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({}, "", "/?section=progress");window.dispatchEvent(new PopStateEvent("popstate"))}}>Прогресс</button>}<button onClick={()=>{window.history.pushState({},"","/?section=schedule");window.dispatchEvent(new PopStateEvent("popstate"))}}>Расписание</button>{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({},"","/?section=materials");window.dispatchEvent(new PopStateEvent("popstate"))}}>Материалы</button>}{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({},"","/?section=templates");window.dispatchEvent(new PopStateEvent("popstate"))}}>Шаблоны</button>}{isAppAdmin&&<button onClick={()=>{window.history.pushState({},"","/?section=admin");window.dispatchEvent(new PopStateEvent("popstate"))}}>Админ</button>}<button onClick={()=>{window.history.pushState({},"","/?section=chat");window.dispatchEvent(new PopStateEvent("popstate"))}}>💬 Сообщения</button><button onClick={()=>{window.history.pushState({},"","/?section=notifications");window.dispatchEvent(new PopStateEvent("popstate"))}}>Уведомления{notificationUnreadCount>0&&<b className="nav-badge">{notificationUnreadCount>99?"99+":notificationUnreadCount}</b>}</button><button onClick={()=>{window.history.pushState({},"","/?section=profile");window.dispatchEvent(new PopStateEvent("popstate"))}}>Настройки</button><button onClick={()=>{window.history.pushState({},"","/?section=guide");window.dispatchEvent(new PopStateEvent("popstate"))}}>🧪 Инструкция тестировщика</button></div></details>{user.isGuest&&<div className="guest-lifecycle-note"><strong>Гостевой режим</strong><span>Это временная анонимная сессия. Доступ к доскам сохранится только пока действует эта гостевая сессия. Для постоянного аккаунта выйдите и зарегистрируйтесь.</span></div>}<nav className="dashboard-nav"><button className="active">Доски</button>{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({}, "", "/?section=students");window.dispatchEvent(new PopStateEvent("popstate"))}}>Ученики</button>}<button onClick={()=>{window.history.pushState({}, "", "/?section=assignments");window.dispatchEvent(new PopStateEvent("popstate"))}}>Задания</button>{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({}, "", "/?section=progress");window.dispatchEvent(new PopStateEvent("popstate"))}}>Прогресс</button>}<button onClick={()=>{window.history.pushState({},"","/?section=schedule");window.dispatchEvent(new PopStateEvent("popstate"))}}>Расписание</button>{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({},"","/?section=materials");window.dispatchEvent(new PopStateEvent("popstate"))}}>Материалы</button>}{accountRole==="teacher"&&<button onClick={()=>{window.history.pushState({},"","/?section=templates");window.dispatchEvent(new PopStateEvent("popstate"))}}>Шаблоны</button>}{isAppAdmin&&<button onClick={()=>{window.history.pushState({},"","/?section=admin");window.dispatchEvent(new PopStateEvent("popstate"))}}>Администрирование</button>}<button onClick={()=>{window.history.pushState({},"","/?section=chat");window.dispatchEvent(new PopStateEvent("popstate"))}}>💬 Сообщения</button><button onClick={()=>{window.history.pushState({},"","/?section=notifications");window.dispatchEvent(new PopStateEvent("popstate"))}}>Уведомления{notificationUnreadCount>0&&<b className="nav-badge">{notificationUnreadCount>99?"99+":notificationUnreadCount}</b>}</button><button className="testing-guide-nav-button" onClick={()=>{window.history.pushState({},"","/?section=guide");window.dispatchEvent(new PopStateEvent("popstate"))}}>🧪 Инструкция</button></nav><div className="boards-heading-row"><div><h1>Мои доски</h1><p>{isRemoteBackendEnabled()?"Уроки, материалы и совместные доски в одном месте.":"Сейчас данные хранятся в этом браузере. Ссылки доступа появятся после подключения сервера."}</p></div><div className="boards-heading-actions">{accountRole==="teacher"?<><label className={`boards-secondary boards-import ${busy?"disabled":""}`}>Импортировать<input type="file" hidden disabled={busy} accept=".orboard,.json,application/json,application/vnd.onlinerepetitor.board+json" onChange={e=>{const f=e.target.files?.[0];e.target.value="";if(f)void chooseImport(f)}}/></label><button className="boards-create" disabled={busy} onClick={()=>void add()}>+ Новая доска</button></>:<span className="student-board-note">Ученик работает только с досками, которые предоставил преподаватель.</span>}</div></div>
 ...
 {manage&&<div className="access-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setManage(null)}}><section className="access-modal"><div className="access-head"><div><h2>Пригласить на доску</h2><p>{manage.title}</p></div><button onClick={()=>setManage(null)}>×</button></div>

 <div className="board-invite-link-panel">
   <div className="board-invite-link-title"><span>🔗</span><div><strong>Ссылка-приглашение</strong><small>Выберите, что сможет делать участник</small></div></div>
   ...
   <div className="board-invite-role-grid"><button className="editor" disabled={busy||!isRemoteBackendEnabled()} onClick={()=>void createLink("editor")}><span>✎</span><b>Редактирование</b><small>Можно менять содержимое доски</small></button><button className="viewer" disabled={busy||!isRemoteBackendEnabled()} onClick={()=>void createLink("viewer")}><span>👁</span><b>Только просмотр</b><small>Без возможности редактирования</small></button></div>
   {!isRemoteBackendEnabled()&&<div className="access-notice">Для ссылок доступа нужна серверная синхронизация.</div>}
   {createdUrl&&<div className="board-invite-created"><div><span>✓</span><strong>Ссылка готова</strong></div><div><input readOnly value={createdUrl} onFocus={e=>e.currentTarget.select()}/><button onClick={()=>void copyCreated()}>⧉ Копировать</button></div></div>}
 </div>
