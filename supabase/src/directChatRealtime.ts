...
export function subscribeDirectChat(userId:string,onChange:()=>void):()=>void{
 ...
 const topic=`realtime:direct-chat:${userId}`;
 ...
 const connect=async()=>{
  ...
  try{
   ...
   ws.onopen=()=>{attempt=0;send("phx_join",{access_token:session.access_token,config:{broadcast:{self:false,ack:false},presence:{enabled:false},postgres_changes:[{event:"*",schema:"public",table:"direct_chat_messages"},{event:"*",schema:"public",table:"direct_chat_members"}]}},topic,joinRef);heartbeat=setInterval(()=>{try{send("heartbeat",{},"phoenix")}catch{schedule()}},25000)};
