...
export function LinkMediaPlayer({item}: {item:Item}){
 ...
 if(info.kind==="youtube"||info.kind==="vimeo"){
  ...
  return <iframe className="link-media-frame" src={info.embedUrl} title={item.mediaTitle||"Видео"} referrerPolicy="strict-origin-when-cross-origin" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen onPointerDown={e=>e.stopPropagation()}/>;
 }
