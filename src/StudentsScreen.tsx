...
export default function StudentsScreen({user,onBack,onOpenBoard}:Props){
 ...
 const persistCard=()=>{if(!student)return;const parsed=tags.split(",").map(x=>x.trim()).filter(Boolean).slice(0,12);saveStudentNote(student.userId,note);saveStudentTags(student.userId,parsed);setStudents(cur=>cur.map(s=>s.userId===student.userId?{...s,note,tags:parsed}:s));setNotice("Карточка ученика сохранена")};
 ...
 return <main className="students-shell">
  ...
  <section className="students-content">
   <nav className="dashboard-nav"><button onClick={onBack}>Доски</button><button className="active">Ученики</button><button disabled>Расписание</button><button disabled>Материалы</button></nav>
