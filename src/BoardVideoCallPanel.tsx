...
export default function BoardVideoCallPanel({boardId,userId,participants}:Props){
 const [state,setState]=useState<CallState>("idle"),[open,setOpen]=useState(false),[chooser,setChooser]=useState(false),[targetId,setTargetId]=useState<string|null>(null),[incomingId,setIncomingId]=useState<string|null>(null),[muted,setMuted]=useState(false),[cameraOff,setCameraOff]=useState(false),[sharingScreen,setSharingScreen]=useState(false),[minimized,setMinimized]=useState(false),[error,setError]=useState("");
 ...
 const closePeer=()=>{clearDisconnectTimer();pc.current?.close();pc.current=null;candidateQueue.current=[]};
 const reset=useCallback(()=>{closePeer();stopMedia();pendingOffer.current=null;setCallState("idle");setOpen(false);setChooser(false);setTargetId(null);setIncomingId(null);setMuted(false);setCameraOff(false);setSharingScreen(false);setMinimized(false);setPosition(null);setIceMode(null);setConnectionRoute("unknown");drag.current=null},[]);
 const media=async()=>{if(stream.current)return stream.current;if(!navigator.mediaDevices?.getUserMedia)throw new Error("Браузер не поддерживает камеру и микрофон");const s=await navigator.mediaDevices.getUserMedia({video:true,audio:true});stream.current=s;cameraTrack.current=s.getVideoTracks()[0]??null;if(localVideo.current)localVideo.current.srcObject=s;return s};
 ...
 const createPeer=async(otherId:string)=>{const s=await media();const iceConfig=await getBoardCallIceConfig();setIceMode(iceConfig.mode);const peer=new RTCPeerConnection({iceServers:iceConfig.iceServers,iceCandidatePoolSize:4});pc.current=peer;s.getTracks().forEach(track=>peer.addTrack(track,s));peer.ontrack=e=>{if(remoteVideo.current)remoteVideo.current.srcObject=e.streams[0]};peer.onicecandidate=e=>{if(e.candidate)void sendBoardCallSignal(boardId,otherId,"candidate",e.candidate.toJSON()).catch(()=>{})};peer.onconnectionstatechange=()=>{
  ...
  if(connectionState==="connected"){clearDisconnectTimer();setError("");setCallState("connected");void detectConnectionRoute(peer);return}
 ...
 return <div className="board-call-control"><button className={`top-button board-call-button ${activeCall?"active":""}`} disabled={state!=="idle"||others.length===0} onClick={()=>others.length===1?void call(others[0].userId):setChooser(v=>!v)} title={others.length?"Видеозвонок участнику этой доски":"Для звонка нужен ещё один участник на доске"} aria-label={activeCall?"Видеозвонок активен":"Начать видеозвонок"}><svg className="board-call-icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="3.5" y="6.5" width="11.5" height="11" rx="3"/><path d="m15 10 4.2-2.4c.6-.35 1.3.08 1.3.77v7.26c0 .69-.7 1.12-1.3.77L15 14"/></svg></button>{chooser&&state==="idle"&&<div className="board-call-chooser"><strong>Позвонить</strong>{others.map(p=><button key={p.userId} onClick={()=>void call(p.userId)}><span>{p.name.trim().charAt(0).toUpperCase()||"U"}</span><b>{p.name}</b></button>)}</div>}{open&&<div className="video-call-layer"><section className={`video-call-card ${minimized?"minimized":""}`} style={style}><header className="video-call-drag-handle" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}><div><strong>{state==="incoming"?`${peerName} звонит`:state==="connected"?peerName:`Соединяем с ${peerName}`}</strong><span>{state==="incoming"?"Входящий звонок на этой доске":state==="connected"?"Звонок на доске":"Устанавливаем соединение…"}</span></div>{state!=="incoming"&&<button className="video-call-minimize" type="button" onPointerDown={e=>e.stopPropagation()} onClick={()=>setMinimized(v=>!v)} title={minimized?"Развернуть":"Свернуть"}>{minimized?"▣":"−"}</button>}</header>{!minimized&&<><div className="video-call-stage"><video ref={remoteVideo} autoPlay playsInline className="video-call-remote"/><video ref={localVideo} autoPlay muted playsInline className="video-call-local"/><div className="video-call-placeholder">{state==="incoming"?"📞":state==="connected"?"":"Подключаем видео…"}</div>{sharingScreen&&<span className="video-call-sharing-badge">Демонстрация экрана</span>}{iceMode&&<span className="video-call-sharing-badge" title={connectionRoute==="relay"?"Звонок фактически передаётся через TURN relay":connectionRoute==="direct"?"WebRTC выбрал прямой маршрут между участниками":iceMode==="turn"?"TURN доступен, определяем фактический маршрут":"TURN не получен, используется прямое ICE-соединение"}>{connectionRoute==="relay"?"Через TURN":connectionRoute==="direct"?"Напрямую":iceMode==="turn"?"TURN доступен":"Прямое соединение"}</span>}</div>{error&&<div className="video-call-error">{error}</div>}<footer>{state==="incoming"?<><button className="video-call-decline" onClick={()=>void hangup()}>Отклонить</button><button className="video-call-accept" onClick={()=>void accept()}>Принять</button></>:<><button className={muted?"active":""} onClick={toggleMute}>{muted?"🔇 Включить":"🎙 Микрофон"}</button><button className={cameraOff?"active":""} onClick={toggleCamera}>{cameraOff?"📷 Включить":"📹 Камера"}</button>{state==="connected"&&<button className={sharingScreen?"active":""} onClick={()=>void shareScreen()}>{sharingScreen?"⏹ Стоп экран":"🖥 Экран"}</button>}<button className="video-call-decline" onClick={()=>void hangup()}>Завершить</button></>}</footer></>}</section></div>}</div>;
}
