...
export default function BoardVideoCallPanel({boardId,userId,participants}:Props){
 ...
 const media=async()=>{if(stream.current)return stream.current;if(!navigator.mediaDevices?.getUserMedia)throw new Error("Браузер не поддерживает камеру и микрофон");try{const s=await navigator.mediaDevices.getUserMedia({video:true,audio:true});stream.current=s;cameraTrack.current=s.getVideoTracks()[0]??null;if(localVideo.current)localVideo.current.srcObject=s;return s}catch(e){const name=(e as DOMException)?.name;if(name==="NotAllowedError"||name==="SecurityError")throw new Error("Разрешите доступ к камере и микрофону в браузере");if(name==="NotFoundError")throw new Error("Камера или микрофон не найдены");if(name==="NotReadableError")throw new Error("Камера или микрофон уже используются другим приложением");throw e}};
