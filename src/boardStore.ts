...
export const touchBoard=async(u:string,id:string,title?:string)=>{if(isRemoteBackendEnabled()){const body: Record<string, unknown> = { updated_at: new Date().toISOString() };if(title?.trim())body.title=title.trim();const rows=await remoteRequest<any[]>(`/rest/v1/boards?id=eq.${encodeURIComponent(id)}&select=id,title,owner_id,created_at,updated_at`,{method:"PATCH",headers:{Prefer:"return=representation"},body:JSON.stringify(body)});if(!rows[0])return null;const role=rows[0].owner_id===u?"owner":(await remoteRequest<any[]>(`/rest/v1/board_members?select=role&board_id=eq.${encodeURIComponent(id)}&user_id=eq.${encodeURIComponent(u)}&limit=1`))[0]?.role||"viewer";return rowToBoard(rows[0],role)}const r=localRole(u,id);if(r!=="owner"&&r!=="editor")return null;let out:BoardSummary|null=null;write(BOARDS_KEY,boards().map(b=>b.id===id?(out={...b,title:title?.trim()||b.title,updatedAt:new Date().toISOString()}):b));return out?{...out,role:r}:null};
