...
export default function BoardsScreen({user,onOpenBoard,onLogout}:Props){
 ...
 <section className="boards-content"><div className="boards-heading-row"><div><h1>Мои доски</h1><p>{isRemoteBackendEnabled()?"Доски синхронизируются через сервер. Для совместной работы отправьте защищённую ссылку.":"Сейчас данные хранятся в этом браузере. Ссылки доступа появятся после подключения сервера."}</p></div><button className="boards-create" disabled={busy} onClick={()=>void add()}>+ Новая доска</button></div><div className="boards-toolbar"><input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Поиск по названию доски"/><span>{loading?"Загрузка…":`${boards.length} досок`}</span></div>
 ...
 <div style={{display:"grid",gap:10,marginBottom:16}}>
   ...
   <span style={{fontSize:13,opacity:.72}}>Создай ссылку с нужными правами и отправь её человеку. После входа или регистрации доступ применится автоматически.</span>
