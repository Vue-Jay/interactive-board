...
export default function ProgressScreen({user,onBack}:Props){
 ...
 return <main className="progress-shell"><header className="students-header"><div><button className="boards-secondary" onClick={onBack}>← Доски</button><div><strong>Прогресс</strong><span>Занятия, задания и результаты учеников</span></div></div><button className="boards-secondary" onClick={()=>void load()}>Обновить</button></header>
  <section className="progress-content"><nav className="dashboard-nav"><button onClick={onBack}>Доски</button><button onClick={()=>{window.history.pushState({},"","/?section=students");window.dispatchEvent(new PopStateEvent("popstate"))}}>Ученики</button><button onClick={()=>{window.history.pushState({},"","/?section=assignments");window.dispatchEvent(new PopStateEvent("popstate"))}}>Задания</button><button className="active">Прогресс</button><button onClick={()=>{window.history.pushState({},"","/?section=schedule");window.dispatchEvent(new PopStateEvent("popstate"))}}>Расписание</button><button disabled>Материалы</button></nav>
